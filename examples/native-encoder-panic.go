...
//
// typedef struct _EcShard {
//     unsigned char **data;
//     unsigned char *dataArray[14];
// } EcShard;
// void setEcShardPointer(EcShard *shard) {
//     shard->data = (unsigned char **)(shard->dataArray);
// }
//
...
func main() {
	...
	var inputs C.EcShard
	//inputs := (**C.uchar)(C.malloc(C.size_t(*dataShards)))
	for i = 0; i < *dataShards; i++ {
		//C.setPointer(inputs, C.int(i), (*C.uchar)(unsafe.Pointer(&shards2[i][0])))
		C.setEcShardPointer(&inputs)
		inputs.dataArray[i] = (*C.uchar)(unsafe.Pointer(&shards2[i][0]))
	}
	var outputs C.EcShard
	//outputs := (**C.uchar)(C.malloc(C.size_t(*parShards)))
	for i = 0; i < *parShards; i++ {
		//C.setPointer(outputs, C.int(i), (*C.uchar)(unsafe.Pointer(&shards2[*dataShards+i][0])))
		C.setEcShardPointer(&outputs)
		outputs.dataArray[i] = (*C.uchar)(unsafe.Pointer(&shards2[*dataShards+i][0]))
	}
	C.encode(&isalEncoder, inputs.data, outputs.data, C.int(actualShardSize))
